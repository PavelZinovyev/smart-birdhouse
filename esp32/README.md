...
Если ESP32 питается только от 3.3V и GND (понижающий преобразователь от 18650/TP4056), **напряжение банки до платы не доходит** — измерять заряд нельзя, в API будет `battery: 0`, `battery_available: false`.

Чтобы показывать процент заряда, нужен **делитель напряжения** от плюса батареи (BAT+ до или после TP4056) на ADC-пин ESP32 (например GPIO 1):

- **Схема:** BAT+ → R1 → ADC-пин → R2 → GND. ADC не должен видеть больше 3.3 V.
- **Пример для 4.2 V max:** R1 = 10 kΩ, R2 = 27 kΩ → на ADC при 4.2 V будет ~3.06 V.
- **Формула:** V_bat = V_adc × (R1 + R2) / R2. В коде в `getBatteryPercent()` замените `vBat = v` на `vBat = v * (R1+R2)/R2` под свои резисторы.

После подключения делителя к GPIO 1 прошейте плату заново; при необходимости подстройте `BATTERY_VOLTAGE_MAX` (4.2) и `BATTERY_VOLTAGE_MIN` (3.0) под ваши пределы.
